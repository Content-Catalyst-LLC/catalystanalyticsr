...
def test_econometrics_contract_files_and_versions():
    assert 'Version: 2.0.0' in (ROOT/'DESCRIPTION').read_text()
...
def test_browser_contract_discloses_non_r_boundary():
    ...
    for token in ["compatible_repository_version:'2.0.0'",'mapped_connected_platform_contract_not_r_execution','identity_not_verified:true','automated_publication:false']:
        assert token in js
