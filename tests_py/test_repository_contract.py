...
def test_versions_and_manifest():
    text=(ROOT/'DESCRIPTION').read_text(); assert re.search(r'^Version:\s*2\.2\.0$',text,re.M)
    m=load('catalyst_analytics_r_manifest.json'); assert m['schema_version']=='3.2.0'; assert m['repository_version']=='2.2.0'; assert m['r_package']['version']=='2.2.0'; assert m['wordpress_demo']['version']=='3.2.0'; assert m['wordpress_demo']['compatible_repository_version']=='2.2.0'
...
def test_policy_evaluation_contracts_validate():
    ...
    fixture=load('tests/fixtures/econometrics_policy_evaluation_contract_v1.json'); assert fixture['repository_version']=='2.2.0'; assert 'difference_in_differences' in fixture['methods']['quasi_experimental']
...
def test_plugin_contract_and_zip():
    ...
    assert 'Version: 3.2.0' in php; assert 'Catalyst Analytics R v2.2.0' in php; assert 'Connected Sustainability Analytics and Decision Platform' in php; assert 'aria-live="polite"' in php
    for token in ["compatible_repository_version:'2.2.0'",'mapped_connected_platform_contract_not_r_execution','identity_not_verified:true','automated_publication:false']: assert token in js
    assert ':focus-visible' in css
    path=ROOT/'dist/catalyst-analytics-r-demo-v3.2.0.zip'; assert path.exists()
    with zipfile.ZipFile(path) as z: assert z.testzip() is None

def test_stable_api_and_docs():
    text=(ROOT/'R/production_readiness.R').read_text(); assert 'econometrics = c(' in text; assert 'econometric_evaluation = "1.0.0"' in text; assert '.catalyst_package_version()' in text; assert 'version = "3.2.0"' in text
