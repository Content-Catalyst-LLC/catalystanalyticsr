...
def test_versions_and_manifest():
    text=(ROOT/'DESCRIPTION').read_text(); assert re.search(r'^Version:\s*1\.6\.0$',text,re.M)
    m=load('catalyst_analytics_r_manifest.json'); assert m['schema_version']=='2.6.0'; assert m['repository_version']=='1.6.0'; assert m['r_package']['version']=='1.6.0'; assert m['wordpress_demo']['version']=='2.6.0'; assert m['wordpress_demo']['compatible_repository_version']=='1.6.0'
...
def test_policy_evaluation_contracts_validate():
    ...
    fixture=load('tests/fixtures/econometrics_policy_evaluation_contract_v1.json'); assert fixture['repository_version']=='1.6.0'; assert 'difference_in_differences' in fixture['methods']['quasi_experimental']
...
def test_plugin_contract_and_zip():
    ...
    assert 'Version: 2.6.0' in php; assert 'Catalyst Analytics R v1.6.0' in php; assert 'Collaborative review and institutional governance' in php; assert 'aria-live="polite"' in php
    for token in ["compatible_repository_version:'1.6.0'",'mapped_institutional_governance_contract_not_r_execution','identity_not_verified:true','automated_publication:false']: assert token in js
    assert ':focus-visible' in css
    path=ROOT/'dist/catalyst-analytics-r-demo-v2.6.0.zip'; assert path.exists()
    with zipfile.ZipFile(path) as z: assert z.testzip() is None

def test_stable_api_and_docs():
    text=(ROOT/'R/production_readiness.R').read_text(); assert 'econometrics = c(' in text; assert 'econometric_evaluation = "1.0.0"' in text; assert 'version = "1.6.0"' in text; assert 'version = "2.6.0"' in text
    assert (ROOT/'docs/econometrics-policy-evaluation.md').exists(); assert (ROOT/'docs/migration-v1.3-to-v1.4.md').exists(); assert (ROOT/'docs/releases/v1.6.0.md').exists()
