...
def test_connected_platform_contract_files_and_versions():
    ...
    assert manifest["repository_version"] == "2.3.0"
    assert manifest["wordpress_demo"]["version"] == "3.3.0"
...
def test_browser_connected_platform_contract():
    ...
    assert re.search(r"^ \* Version:\s*3\.3\.0$", php, re.M)
    for token in ["Connected Sustainability Analytics and Decision Platform", "Build connected platform", 'aria-live="polite"']:
        assert token in php
    for token in ["compatible_repository_version:'2.3.0'", "mapped_connected_platform_contract_not_r_execution", "automated_decision_authorization:false", "automated_publication:false"]:
        assert token in js
    with zipfile.ZipFile(ROOT / "dist/catalyst-analytics-r-demo-v3.3.0.zip") as archive:
