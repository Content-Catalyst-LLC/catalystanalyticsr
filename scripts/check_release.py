...
REPOSITORY_VERSION='2.0.1'; PLUGIN_VERSION='3.0.0'; MANIFEST_VERSION='3.0.0'
...
def main():
    if not re.search(r'^Version:\s*2\.0\.1$',read('DESCRIPTION'),re.M): fail('DESCRIPTION version mismatch')
    ...
    required=['R/connected_platform.R','man/connected_platform.Rd','man/export_connected_platform.Rd','schemas/catalyst_analytics_r_connected_platform.schema.json','schemas/catalyst_analytics_r_connected_platform_export.schema.json','schemas/catalyst_analytics_r_connected_platform_demo_export.schema.json','examples/connected_platform_input.json','outputs/example_connected_platform_export.json','outputs/example_browser_connected_platform_export.json','tests/fixtures/connected_platform_contract_v2.json','tests/testthat/test-connected-platform.R','tests_py/test_connected_platform_contract.py','docs/connected-sustainability-analytics-decision-platform.md','docs/migration-v1.6-to-v2.0.md','docs/releases/v2.0.0.md','docs/releases/v2.0.1.md',f'dist/catalyst-analytics-r-demo-v{PLUGIN_VERSION}.zip']
    missing=[p for p in required if not (ROOT/p).exists()]
    if missing: fail(f'Missing v2.0.1 files: {missing}')
    handoff_source=read('R/public_api_handoffs.R')
    for token in ('workspace_handoff','workspace_runtime_request','workspace_controls_execution_environment','runtime_execution_not_performed_by_package'):
        if token not in handoff_source: fail(f'Workspace handoff implementation missing: {token}')
    handoff_fixture=load('tests/fixtures/public_api_handoff_contract_v1.json')
    if 'workspace' not in handoff_fixture['targets'] or len(handoff_fixture['targets']) != 7: fail('Workspace handoff target contract mismatch')
    ...
    for token in ('connected_platform = "2.0.0"','connected_platform_export = "2.0.0"','connected_api = "2.0.0"','.catalyst_package_version()','version = "3.0.0"'):
    ...
    print('Catalyst Analytics R v2.0.1 release contract passed.')
